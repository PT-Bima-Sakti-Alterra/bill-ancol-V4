...
var
  Fpiutanggolongan: TFpiutanggolongan;
...
procedure TFpiutanggolongan.FormShow(Sender: TObject);
begin
...
procedure TFpiutanggolongan.btnOkeClick(Sender: TObject);
var
keterangan:String;
i:integer;
...
begin
  ...
  if( YearOf(tanggal.date) <  YearOf(periode2.date) ) then
  begin
      MessageDlg('Tahun pada tanggal tidak boleh lebih kecil dari Periode/ S.d ' +
         'DRD !!', mtError, [MbOk], 0); 
      exit;
  end;
...
TRY
  ...
  TRY
    ...
    report.LoadFromFile(GetCurrentDir+'\report\rekappiutanggolongan.fr3');
...
procedure TFpiutanggolongan.btnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TFpiutanggolongan.kodewilPropertiesChange(Sender: TObject);
begin
  wilayah.ItemIndex:=kodewil.ItemIndex;
end;

procedure TFpiutanggolongan.wilayahPropertiesChange(Sender: TObject);
begin
  kodewil.ItemIndex:=wilayah.ItemIndex;
end;

procedure TFpiutanggolongan.kodeflagPropertiesChange(Sender: TObject);
begin
  flag.ItemIndex:=kodeflag.ItemIndex;
end;

procedure TFpiutanggolongan.flagPropertiesChange(Sender: TObject);
begin
  kodeflag.ItemIndex:=flag.ItemIndex;
end;

procedure TFpiutanggolongan.cekwilClick(Sender: TObject);
begin
   if(cekwil.Checked=true)then
  begin
...
procedure TFpiutanggolongan.cekFlagClick(Sender: TObject);
begin
  if(cekFlag.Checked=true)then
  begin
...
procedure TFpiutanggolongan.FormCreate(Sender: TObject);
begin
...
procedure TFpiutanggolongan.cekstatusClick(Sender: TObject);
begin
  if(cekstatus.Checked=true)then
  begin
       status.Enabled:=true; 
...
procedure TFpiutanggolongan.cekrayonClick(Sender: TObject);
begin
 if(cekrayon.Checked=true)then
  begin
...
procedure TFpiutanggolongan.cekgolonganClick(Sender: TObject);
begin
 if(cekgolongan.Checked=true)then
  begin
...
procedure TFpiutanggolongan.koderayonPropertiesChange(Sender: TObject);
begin
  namarayon.ItemIndex:=koderayon.ItemIndex;
end;

procedure TFpiutanggolongan.namarayonPropertiesChange(Sender: TObject);
begin
  koderayon.ItemIndex:=namarayon.ItemIndex;
end;

procedure TFpiutanggolongan.kodegolPropertiesChange(Sender: TObject);
begin
  golongan.ItemIndex:=kodegol.ItemIndex;
end;

procedure TFpiutanggolongan.golonganPropertiesChange(Sender: TObject);
begin
  kodegol.ItemIndex:=golongan.ItemIndex;
end;

procedure TFpiutanggolongan.ceklembarClick(Sender: TObject);
begin
 if(ceklembar.Checked=true)then
  begin
...
procedure TFpiutanggolongan.Action3Execute(Sender: TObject);
begin
report.LoadFromFile(GetCurrentDir+'\report\rekappiutanggolongan.fr3');
report.DesignReport(true);
end;

procedure TFpiutanggolongan.tanggalPropertiesChange(Sender: TObject);
begin
periode1.date:=EncodeDateTime(1989, 1, 5, 1,2, 3, 4);
periode2.date:=EncodeDateTime(YearOf(tanggal.date), MonthOf(tanggal.date), 5, 1,
    2, 3, 4);
end;
