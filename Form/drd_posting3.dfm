object udrd_posting3: Tudrd_posting3
  Left = 536
  Top = 257
  BorderStyle = bsNone
  Caption = 'DRD'
  ...
  object Qrek: TMyQuery
    SQLRefresh.Strings = (
        ...
        'SELECT r.kodearea,r.area,c.koderayon,r.namarayon,SUM(c.flagpubli' +
        'sh="1") AS publish,SUM(c.flagpublish="0") AS belumpublish,COUNT(' +
        'c.nosamb) AS pelanggan,'
      ''
      'COALESCE(SUM( IF(c.flagpublish="1",c.pakai,0)),0) AS pakai,'
      
        'COALESCE(SUM( IF(c.flagpublish="1",c.biayapemakaian,0)),0) AS bi' +
        'ayapemakaian,'
      
        'COALESCE(SUM( IF(c.flagpublish="1",c.administrasi,0)),0) AS admi' +
        'nistrasi,'
      
        'COALESCE(SUM( IF(c.flagpublish="1",c.pemeliharaan,0)),0) AS peme' +
        'liharaan,'
      'COALESCE(SUM( IF(c.flagpublish="1",c.rekair,0)),0) AS rekair'
      ''
      
        'FROM drdposting:periode c LEFT JOIN rayon r ON c.koderayon=r.`ko' +
        'derayon` LEFT JOIN pelanggan p ON c.nosamb=p.`nosamb` '
  ...
  object report: TfrxReport
    ...
    ReportOptions.LastChange = 42949.694367974500000000
    ...
    object Page3: TfrxReportPage
      ...
      object ReportSummary1: TfrxReportSummary
        ...
        object Memo29: TfrxMemoView
          ...
          HAlign = haRight
          Memo.UTF8 = (
            '[SUM(<rek."belumpublish">,MasterData1)]')
          ParentFont = False
        end
