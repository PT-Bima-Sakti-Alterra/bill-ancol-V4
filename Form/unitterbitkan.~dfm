object uterbitkan: Tuterbitkan
  ...
  object Qpelmain: TMyQuery
    SQLRefresh.Strings = (
      ...
      'k.kodecabang,'
      'p.kodegol,'
      'p.koderetribusilain as kodeipl,'
      'p.kodediameter,'
      'p.kodekolektif,'
      'p.serimeter,'
      ...
      'per.tglmulaidenda3,'
      'per.tglmulaidenda4,'
      'per.tglmulaidendaperbulan'
      ''
      ''
      'FROM pelanggan p '
      
        '-- LEFT JOIN (SELECT kodegol,golongan FROM golongan GROUP BY kod' +
        'egol) g ON p.kodegol=g.kodegol'
      
        '-- LEFT JOIN (SELECT  kodediameter,ukuran FROM diameter GROUP BY' +
        ' kodediameter ) d ON p.kodediameter=d.kodediameter'
      'LEFT JOIN stan s ON p.nosamb=s.nosamb '
      'LEFT JOIN periode per ON per.periode=:periode'
      'LEFT JOIN rayon r ON p.koderayon=r.koderayon'
      'LEFT JOIN kelurahan k ON p.kodekelurahan=k.kodekelurahan'
      ''
      'WHERE '
      
        '(p.status<>"0" OR (p.status="0" and DATE_FORMAT(p.tglputus,"%Y%m' +
        '")>=:periode) OR (p.status="0" and p.tglputus IS NULL)) '
      'AND p.nosamb NOT IN (SELECT nosamb FROM drd:periode)'
      'ORDER BY p.nosamb ASC')
    Connection = DM.Conn
    Left = 128
    Top = 328
  end
  object Qcek: TMyQuery
    SQLRefresh.Strings = (
      ...
      '  left join rayon r On p.koderayon=r.koderayon '
        ...
        '  LEFT JOIN `bypemeliharaan_lain` b2 ON p.kodepemeliharaanlain=b' +
        '2.`kode`'
      
        '  LEFT JOIN `byretribusi_lain` b3 ON p.koderetribusilain=b3.`kod' +
        'e`'
      ''
  ...
  object data_: TdxMemData
    ...
    object data_statuspel: TStringField
      FieldName = 'statuspel'
      Size = 100
    end
    ...
    object data_kodeipl: TStringField
      FieldName = 'kodeipl'
      Size = 100
    end
  end
