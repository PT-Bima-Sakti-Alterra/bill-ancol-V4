...
procedure Tulppharian7.cxButton2Click(Sender: TObject);
begin
close;
end;

procedure Tulppharian7.cxButton1Click(Sender: TObject);
var
keterangan:String;
begin

  report.LoadFromFile(GetCurrentDir+'\Report\rekaplppairbulanberjalan.fr3');
 ...
 begin
  ...
  begin
      ...
      keterangan:='WILAYAH : '+wilayah.Text+char(13)+keterangan;   
  end;
  ...
  Qrek.ParamByName('periodedrd').AsString:=FormatDateTime('YYYYMM',IncMonth(periode.date,-1));
  Qrek.Open;
  ...
  TfrxMemoView(report.FindObject('memo5')).Memo.Text:='( KHUSUS DRD '+FormatDateTime('MMM YYYY',IncMonth(periode.date,-1))+' )';
...
procedure Tulppharian7.FormCreate(Sender: TObject);
begin
periode.date:=EncodeDateTime(YearOf(date), MonthOf(date), 5, 1, 2, 3, 4);
end;

procedure Tulppharian7.F5Execute(Sender: TObject);
begin
report.LoadFromFile(GetCurrentDir+'\report\rekaplppairbulanberjalan.fr3');
  report.DesignReport(true);
end;

procedure Tulppharian7.cekkasirClick(Sender: TObject);
begin
  if(cekkasir.Checked=true)then
  begin
      kasir.Enabled:=true;
...
procedure Tulppharian7.cekshiftClick(Sender: TObject);
begin
 if(cekshift.Checked=true)then
  begin
      shift.Enabled:=true; 
...
procedure Tulppharian7.FormShow(Sender: TObject);
begin
ucombo.isikombo();
...
procedure Tulppharian7.cekgolClick(Sender: TObject);
begin
 if(cekgol.Checked=true)then
  begin
...
procedure Tulppharian7.kodegolPropertiesChange(Sender: TObject);
begin
golongan.ItemIndex:=kodegol.ItemIndex;
end;

procedure Tulppharian7.golonganPropertiesChange(Sender: TObject);
begin
...
procedure Tulppharian7.cekwilClick(Sender: TObject);
begin
if(cekwil.Checked=true)then
  begin
...
procedure Tulppharian7.kodewilPropertiesChange(Sender: TObject);
begin
wilayah.ItemIndex:=kodewil.ItemIndex;
end;

procedure Tulppharian7.wilayahPropertiesChange(Sender: TObject);
begin
kodewil.ItemIndex:=wilayah.ItemIndex;
end;

procedure Tulppharian7.cekloketClick(Sender: TObject);
begin
 if(cekloket.Checked=true)then
  begin
...
procedure Tulppharian7.kodeloketPropertiesChange(Sender: TObject);
begin
loket.ItemIndex:=kodeloket.ItemIndex;
end;

procedure Tulppharian7.loketPropertiesChange(Sender: TObject);
begin
kodeloket.ItemIndex:=loket.ItemIndex;
end;

procedure Tulppharian7.cekwilloketClick(Sender: TObject);
begin
if(cekwilloket.Checked=true)then
  begin
...
procedure Tulppharian7.kodewil2PropertiesChange(Sender: TObject);
begin
wilayah2.ItemIndex:=kodewil2.ItemIndex;
end;

procedure Tulppharian7.wilayah2PropertiesChange(Sender: TObject);
begin
