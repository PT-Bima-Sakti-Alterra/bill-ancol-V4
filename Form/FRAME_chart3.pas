...
procedure TFRchart3.cxButton1Click(Sender: TObject);
var
judul:String;
begin
...
Qcek.close;
Qcek.SQl.Clear;
Qcek.SQL.Add('SELECT DAY(waktubayar) AS tanggal,SUM(total) AS total FROM nonair WHERE flaglunas="1" AND MONTH(waktubayar)=:bulan AND YEAR(waktubayar)=:tahun AND jenis<>"AIR TANGKI" ');
...
Qcek.SQL.Add('GROUP BY DATE(waktubayar) ASC');
...
procedure TFRchart3.kodeloketPropertiesChange(Sender: TObject);
begin
loket.ItemIndex:=kodeloket.ItemIndex;
end;

procedure TFRchart3.loketPropertiesChange(Sender: TObject);
begin
kodeloket.ItemIndex:=loket.ItemIndex;
end;

procedure TFRchart3.cxButton2Click(Sender: TObject);
begin
...
procedure TFRchart3.cekloketClick(Sender: TObject);
begin
 if(cekloket.Checked=true)then
    loket.Enabled:=true
  else
